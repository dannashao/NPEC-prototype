...
def check_investigation():
    ...
    try:
        ...
        try:
            ...
            if not is_new_record and result and result.get('created_at'):
            ...
            for field, value in fields.items():
                if value is not None:  # Skip null values
                    ...
                    if not backend_field:
                        app.logger.warning(f"No backend mapping found for field: {base_field}")
                        continue
                    ...
                    update_values.append(fields[base_field])
                    
                    # Add the binding if it exists
                    binding_field = f"{base_field}_binding"
                    if binding_field in fields:
                        update_fields.append(f"{db_field}_BINDING = %s")
                        update_values.append(fields[binding_field])
                            ...
                            if backend_field:
                                for db_key, backend_value in DB_TO_BACKEND.items():
                                    if backend_value == backend_field:
                                        ...
                                        insert_values.append(fields[base_field])
                                        
                                        # Add the binding if it exists
                                        binding_field = f"{base_field}_binding"
                                        if binding_field in fields:
                                            insert_fields.append(f"{db_field}_BINDING")
                                            insert_values.append(fields[binding_field])
                                        break
